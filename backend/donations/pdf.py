...
def _add_ngo_data(start_y: int, c: Canvas, cause: Cause | None, ngo: Ngo):
    start_ngo_y: int = start_y - 289
...
def _add_donor_data(start_y, c: Canvas, donor: Donor):
    donor_block_x: int = 70
    donor_block_y: int = start_y - 57

    # father's initial
    c.drawString(donor_block_x + 230, donor_block_y, donor.initial)

    # the last name
    last_name: str = donor.l_name
    if len(last_name) > 34:
        c.setFontSize(10)
    ...
    # THIRD ROW
    #
    third_row_y: int = donor_block_y - 45

    # the street
    donor_address: dict = donor.get_address()

    street: str = donor_address["str"]
    if len(street) > 40:
        c.setFontSize(8)
    ...
    # număr
    street_number: str = donor_address["nr"]
    ...
    # =======================================
    # FOURTH ROW
    fourth_row_y: int = donor_block_y - 67
    ...
    # cnp
    cnp_x: int = donor_block_x + 266
    cnp_y: int = donor_block_y - 10
...
def _add_signature_to_pdf(c: Canvas, signature: str, new_height: int = 30):
    ...
    byte_image: BytesIO = BytesIO(base64.b64decode(signature))

    # Create an ImageReader object from the byte data
    img: ImageReader = ImageReader(byte_image)

    # Calculate size and scale to fit the desired height
    img_width: int
    img_height: int
    img_width, img_height = img.getSize()

    scale_factor: float = new_height / img_height
    new_width: float = img_width * scale_factor

    # Draw the image on the PDF canvas at the specified position
    c.drawImage(img, 166, 93, width=new_width, height=new_height, mask="auto")
...
def _initialize_pdf_canvas(packet):
    ...
    background: ImageReader = ImageReader(form_image_path)
...
def _add_year_to_pdf(c: Canvas, start_x: int, start_y: int):
    # the year
    # this is the previous year
    year: str = str(datetime.now().year - 1)
    for letter in year:
...
def create_cause_pdf(cause: Cause | None, ngo: Ngo):
    ...
    packet = TemporaryFile(mode="w+b")

    c: Canvas = _initialize_pdf_canvas(packet)
